  ...
  const submitTriagem = async () => {
    ...
    try {
      ...
      if (response.data?.job_id) {
        router.push(`/resultado?job_id=${response.data.job_id}`);
      }
    } catch (e: any) {
      console.error(e);
      Alert.alert('Erro', e.response?.data?.detail || 'Erro ao iniciar análise');
    } finally {
        ...
        {step === 3 && (
          <View className="gap-6">
            ...
            <View className="bg-slate-50 p-6 rounded-2xl border-2 border-slate-100 gap-4 flex-col mt-2">
               ...
               <TextInput 
                 multiline
                 className="bg-white p-4 h-24 rounded-xl border border-slate-200 focus:border-blue-500" 
                 placeholder={t('form.q1Placeholder')}
                 value={concerns}
                 onChangeText={setConcerns}
               />

               <Text className="font-bold text-slate-700 mt-4">{t('form.q2Label')}</Text>
               <TextInput 
                 multiline
                 className="bg-white p-4 h-24 rounded-xl border border-slate-200 focus:border-blue-500" 
